...
#define KCHAR_BIT  8 //ModForKernel: set it for now in this environment -- need to find a corresponding var in kernel
...
u32 rotl32(u32 n, unsigned c) {
    const unsigned mask = KCHAR_BIT * sizeof(n) - 1;
    c &= mask & 31;
    return (n << c) | (n >> ((-c) & mask));
}
...
//ModForKernel: put random number for now - it works in this environment
#define KINT_MAX 9223372036854775807
#define KINT_MIN -9223372036854775807

// 2's complement detection
//#if (INT_MIN != -INT_MAX)
#if (KINT_MIN != -KINT_MAX)
